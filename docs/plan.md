...
- [ ] **路線B': ExIt(探索の蒸留)** — 探索はBCに58.1%で勝つと実証済み=探索の決定はBCより良い教師。
  v4.3a-fixed2同士の自己対戦決定を収集→bc_x1を学習→その上に探索を載せ直す(AlphaZero式方策改善)。
  1世代+40〜60 Elo見込み。重い場合はGoogle Cloudクーポン($3,000)投入
  - 1200分解(07-16): 現平衡予測~1065 → 必要+135 ≈ ExIt×1〜2世代(+56×n) + ミラー支配(+35) + tech(+35)。
    終盤はカウンターメタ提出(8/11メタスナップショット→8/14第2枠)
