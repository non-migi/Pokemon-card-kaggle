...
**13敗中9試合・のべ19局面**(保守的カウント。自動昇格と区別できない境界事例まで含めると
最大12試合・34局面)。89739398では同じ死に方を3連続(420/360/540ダメージでGrimmsnarlEX 3体)。
...
**修正可能性**: 対象局面数(19〜34/13敗)は過去のAZ00x系(13〜29局面/315戦)より桁違いに高頻度で、
...
89739398の3局面を再現することを確認済み)。
残課題: 90471729が0件(全て「不明=自動昇格」)なのは自発Switchログ形式の取りこぼしの疑いあり、
ルール実装前に手動検算する。
