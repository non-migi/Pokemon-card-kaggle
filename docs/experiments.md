...
### 副産物: 相手専用モデルの配線(`opp_policy`)

仮説検証のため、ロールアウト内で相手だけ別モデルに操縦させる機構を実装した(設定なしは完全no-op)。
仮説自体は外れたが、機構は「壁を強くする」用途と、belief連動の相手モデル切替に再利用できる。
`agents/*.json`の`opp_model`キー、`src/ptcg/opp_policy.py`、`bc_search._policy_act(od, my_index)`。
unit 11件追加・全105件通過。
