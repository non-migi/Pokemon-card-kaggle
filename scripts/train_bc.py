...
出力: submission/ptcg/policy_params.npz(numpy推論用) + policy_vocab.py

使い方:
    .venv/bin/python scripts/train_bc.py --data data/bc/pairs_0709.jsonl.gz --epochs 3
"""
...
def main():
    ...
    np.savez_compressed(
        os.path.join(ROOT, "submission", "ptcg", "policy_params.npz"),
        emb=sd["emb.weight"],
    ...
    with open(os.path.join(ROOT, "submission", "ptcg", "policy_vocab.py"), "w") as f:
        f.write('"""BC方策のカード語彙(train_bc.pyが自動生成)。"""\n\n')
        f.write(f"POLICY_FEATURE_VERSION = {PF.POLICY_FEATURE_VERSION}\n")
        f.write(f"CARD_VOCAB = {vocab}\n")
    print("exported -> submission/ptcg/policy_params.npz, policy_vocab.py")
