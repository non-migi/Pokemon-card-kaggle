...
1. ~~**Grim専用スパーリング相手**~~ → **完了(07-25)**。`models/bc_grim/`(164万判断/100チーム、
   holdout top1 **70.25%**)。純BC 400戦で **旧壁52.8% → 新壁35.5%(Top8型) / 39.8%(canonical型)**、
   本番実測25.7%のCI内に入った。**旧壁は本番CIの外**で無効と実証。
   使い方: `build/wall-grim-top8-bc`(主壁) / `build/wall-grim-canonical-bc`(副壁)。
   → **次はこの壁で対Grim 35.5%を上げる施策を測る**。ここが1100への唯一の道。
   同じ手順で`bc_dragapult`(20.8万判断)/`bc_kangaskhan`(70.3万判断)も作れる。
...
- Claude Code (07-25〜): **対Grim改善レーン**。`build/v4.3a-bc`(純BC) vs
  `build/wall-grim-top8-fixed2`(探索付きGrim壁) 200戦を実行中(fixed-worlds、-j 8、約40分)。
  基準は同じ純BC自陣 vs 純BC壁の **35.5%/400**。壁に探索を足して実物(本番25.7%)へ
  どれだけ近づくかを測る。両者探索の400戦版は重すぎて一度停止したので採らない。
