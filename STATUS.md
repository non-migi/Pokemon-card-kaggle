...
最終更新: 2026-08-10 18:57 JST (Claude Code) — 🚨 **v5.4g提出で押し出されたのは v5.0g ではなく
v5.1g(837.8)。activeが v5.4g + v5.0g(探索版760.0)になり、1332位/760.0 まで後退した。**
詳細は下の「ラダー状況」。以下は08-10午前までの記録。
...
0. 🚨 **押し出し事故の復旧を最優先で判断する**(ユーザー承認が必要)。
   いま銅ラインから-79.9点なのは**探索版v5.0g(760.0)がactiveに残っているせい**。
   もう1回提出すればv5.0gが落ちる。候補は `v5.5g`(bc_grim4、A/BはADOPT済み)。
   ⚠️ ただし `build/v5.5g` は**未組立て**(あるのは`build/v5.5g-bc`のみ)。
   提出するなら `ptcglab.build v5.5g` → 両席検証 → 提出の順。
   **判断材料**: v5.5gは本番未検証。安全側に倒すなら v5.1g(837.8で実績あり)の
   再提出でもv5.0gは落とせる。どちらを出すかはユーザーが決める。
...
- (空き) — 08-10 18:57時点で実行中のジョブなし(`ps`で確認)。
  bc_grim4のA/Bは16:54に完走済み(48.75%/400、ADOPT)。`build/v5.5g`は未組立て。
