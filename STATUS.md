...
最終更新: 2026-07-25 13:20 JST (Claude Code) — **本番315戦の対面別分析で頭打ちの原因を特定**:
ミラーは49.6%で健全、対Grim 25.7% / Dragapult 23.1% / Kang 37.5%が壊滅。ローカルmeta加重評価は無効。
...
## ラダー状況(2026-07-25 04:04 JST公式CSV、active = 最新2提出のみ)

| 提出 | active | 内容 | ライブレート / 公開対戦 |
|---|---|---|---|
| **v4.3a** (sub 54731784) | **yes** | bc_v2 BCS + **canonical Top Alakazam** | **839.1** / **315戦157勝158敗 (49.8%)** — 871.7から低下 |
| **v4.2t** (sub 54688865) | **yes** | bc_v2 BCS + Great Tusk–Crustle mill | **699.9** / 255戦125勝130敗 (49.0%) |
...
1. **【最優先・本命】Grim専用のスパーリング相手を作る**: 07-16〜07-24のDaily Top(未使用の
   新しいcorpusが9日分ある)からGrim使用者のエピソードだけを抽出→`bc_grim`をBC学習。
   これを壁にして初めて「対Grimの改善」が測れる。今の`bc_v2`操縦Grim壁は無効(88.95%の嘘の出所)。
   同様に`bc_dragapult`も。これが無いと以降の全施策が測定不能。
2. **【即時・ノーリスク】提出枠を使う**: v4.2t(699.9)は最新2提出の下位。1回提出すれば
   v4.2tだけが落ち、v4.3a(839.1)は残る。候補はHammer4 deck(Top8のMajkel/Yushinと同一60枚、
   純BC 54.17%/300)。同一方策・デッキのみ差分なので、本番でのdeck A/Bとして情報価値が高い。
   締切8/16まで残22日=逐次ラダー試験は実質3回しか回せない。今日始めないと2回になる。
