...
**最終順位 429位 / 6,807チーム、スコア 905.8**(チームスコア = 最新2提出のmax)。
メダル境界(Kaggle標準: 金=上位10+0.2% / 銀=上位5% / 銅=上位10%):

| 区分 | 順位境界 | 境界スコア | 我々との差 |
|---|---:|---:|---:|
| 金 | 23位 | 1130.9 | −225.1 |
| 銀 | 340位 | 924.0 | **−18.2** |
| **銅** | **680位** | **853.7** | **+52.1(圏内)** |

首位は Luca 1398.2。08-10時点(1332位/760.0)から **903位**回復した。

**実際の最終ペア**(STATUS下段の「v5.12g×2本」記述は締切直前に上書きされている点に注意):
| 枠 | 提出 | sub_id | 提出時刻(UTC) | 最終戦数 | 最終score |
|---|---|---|---|---:|---:|
| 1 | **v6.0o(復活)** | **55565273** | 08-16 23:38:51 | 1000 | **905.8** ← チームスコア |
| 2 | v5.12g 2本目 | 55565063 | 08-16 23:27:48 | 1000 | 834.6 |

締切11分前にユーザー判断で v6.0o を再々提出し(説明文 "restored by user decision")、
v5.12g 1本目(55538028、最終843.6)が押し出された。**結果としてこの復活判断が正しかった**:
v5.12g系は2本とも834〜844で銅ラインをわずかに下回り、v6.0oだけが銅圏内に入った。

**収束の経過**(results/final_convergence_log.csv、6時間間隔):
v6.0oは初日6戦862.9 → 50戦800 → 300戦859 → 600戦873 → 08-29以降875〜909の帯で振れ、
09-01 07:55 UTC の観測以降 905.8 で凍結。v5.12g 2本目は 300戦831 → 600戦798 → 08-28に759まで下げた後、
最後の3日で 765→835 に戻して凍結(いずれも1000戦超、正確な総戦数はAPI上限で不明)。
⚠️ 訂正(09-05): episodes=1000 は**APIの一覧上限**であり対戦停止ではない。episodes=1000のままレートが
875→909 / 765→835 と動いているのが証拠。実際の対戦は **08-31 23:44 UTC(締切+15日)まで継続**し、以後ラダー全体が停止して凍結した。

**教訓(Writeup用)**:
1. 「Grim捕食依存だから停滞する」という差し替え根拠は**31戦の標本で下した早計**だった。
   同一ビルドの3回目は1000戦で905.8に到達し、初代(54戦64.8%/862)の実力を裏付けた。
   締切直前の31戦での判断は、当リポジトリ自身の「早期scoreを読まない」則に反していた。
2. 一方で v5.12g は独立2試行が 834.6 / 843.6 とよく一致し、**bc_grim7系の真値は約840**と
   確定した(銅ライン853.7に10〜20点届かない)。アーキタイプ分散(Grim系+Ogerpon系)の
   ペア構成が最終的に効いた。
3. 終了条件は「締切後約15日(08-31 23:44 UTC)でラダー全体が停止」。episodes一覧の1000件はAPI上限で、対戦停止ではなかった。

(以下は締切前の記録。最終ペアは上記のとおり v6.0o + v5.12g 2本目)
## ✅ 決着: 2枠目を v5.12g の2本目に差し替えた(08-17 08:27 JST、締切32分前)

**最終ペア = v5.12g × 2本(sub 55565063 + 55538028)**、両方COMPLETE。v6.0o再(55538007)は押し出し。
事前登録の規則(40戦チェックで650未満なら差し替え)を機械適用した。実際は31戦563.3で、
マッチングが5.5時間で2戦まで鈍化し40戦到達が見込めなかったため、31戦時点の値で適用。

**根拠**: v6.0oの862は**Grim捕食に依存**していた(対Grim 18戦83.3% / 非Grim 35戦54.3%)。
再提出は**31戦でGrim系との遭遇ゼロ**、563.3と初期値600すら回復できず、Grimの多い高レート帯へ
登る足がかりを掴めない状態だった。最終スコアが2提出のmaxである以上、
**実績のある万能型(62.5%/32戦、最高883.8)の独立2試行**が、強い1本+停滞した専門型を上回ると判断。

(以下は判断前の記録)
## ⏳ (決着済み)保留中だった判断: 2枠目(v6.0o再)を差し替えるか

現況(08-16 11:50 JST): **チームスコアは max なので 883.8 = v5.12g が採用中**(銅843.9超え)。
| 提出 | 戦数 | 勝率 | score |
|---|---:|---:|---:|
| v5.12g(1枠目、55538028) | 32戦 | **62.5%** | **883.8** |
| v6.0o再(2枠目、55538007) | 20戦 | 40.0% | 545.7 |
| (参考)v6.0o初代(55519939) | 54戦 | 64.8% | 862到達 |

⚠️ v6.0o再は**初代とバイト単位で同一のビルド**(tarball mtime=初代提出時刻、npzのSHAがbc_ogerpon
と一致、config={"algo":"bc"})。それでも初代64.8%に対し再提出は20戦40.0%と乖離(p≈0.055、有意ではない)。
仮説: **最終日は低レート帯に各チームの最強新規提出が溢れて激戦区化している**。
ただし同条件でv5.12gは62.5%なので「今日のプールにはv5.12gの方が強い」とも読める。

**判断ルール(ユーザー合意済み)**: v6.0o再が**40戦に達した時点で再評価**し、
- なお **650未満なら2枠目を差し替える**(候補: v5.12gのもう1本 — 最終スコアがmaxである以上
  「最良エージェントの独立2回試行」は数学的に有利。アーキタイプ分散は失う)
- 回復していれば静観(初代で862到達の実績があり、締切後2週間で収束し直す時間はある)

⚠️ 新規提出は**古い方**を押し出すので、v6.0o再(23:39:48)が外れ**v5.12g(23:40:59)は残る**。
2枠目だけを安全に差し替えられる。40戦到達は本日15〜18時JSTの見込み(約6戦/時)。

## 最終ペア(両方 COMPLETE 検証済み)

## 最終ペア(両方 COMPLETE 検証済み)

| 枠 | 提出 | sub_id | 中身 | 根拠 |
|---|---|---|---|---|
| 1 | **v5.12g** | **55538028** | bc_grim7(20日347万判断/470チーム/語彙211/**holdout 75.6%**)+ohko_guard(GR001/2)+純BC、Grim合意60枚 | ミラーA/B **56.87%/400 [52.0–61.6]** 両席56%超 = 日次更新系で初めてCIが50%を跨がない優位 |
| 2 | **v6.0o再** | **55538007** | bc_ogerpon(39万判断/86チーム)+純BC、Ogerpon主軸60枚 | ラダー実測 **54戦64.8%**(最高862.2)。v6.1o(43戦51.2%)との直接対決で事前規則により選抜 |

**設計意図**: アーキタイプ非相関のペア。最終スコアは2提出のmaxなので、
Grim系(Alakazam/Lucario/Dragapultに強い、Ogerponに弱い)とOgerpon系(Grim族を狩る、他対面は未知)を
組み合わせて上振れの機会を最大化した。**以後は締切後約2週間の自動対戦で収束し最終順位が確定**
(docs/plan.md:26)。**こちらの作業は完了。残るはStrategy Writeup(締切09-13)のみ。**

## 最終盤の主要な確定事項

- **確定レシピ**: epochs12 + ep8以降LR半減 + export-best。LR減衰がep7 75.1%→ep11 75.6%と
  頭打ちを解消(e12単体では ep7以降0.4pt幅の振動のみだった)
- **反証済み(既定に入れない)**: モデルsoup(44.41%=異初期化の平均は関数を壊す。emb相関ほぼ0が証拠)/
  エリート加重(+0.13pt)/ 容量拡大HID256(+0.2pt)/ ルールGR003・GR004・ミラー脅威拡張
- **忠実度の天井は74-75%付近**(lossは下がり続けるがholdoutは頭打ち)。ただしLR減衰で75.6%まで到達
- **guardの効果はラダーでも証明されず**(対Ogerpon/Lopunny合算16.7%)。害もないので既定のまま
- v5.11g対面別(75戦): ミラー59.3% / Alakazam 63.6% / Lucario 62.5% / **Ogerpon 0%(5戦)**
  → この弱点をOgerpon枠が補完する構成にした
- データは20日分(08-15分はUTC日が終わっておらず未公開。1日分の限界価値は測定不能と判断し待たず)
