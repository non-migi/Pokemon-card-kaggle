...
最終更新: 2026-08-02 02:00 JST (Claude Code) — **環境が入れ替わっていたのでアーキタイプごと乗り換えた**。
①07-31 Daily Top(411,207判断)で上位帯は **オーロンゲ68.4%/89チーム・フーディン3.1%/19チーム**。
`bc_v2`は07-01〜07-11(フーディン全盛期)の学習物で、**モデル化している環境自体が消滅していた**。
②`models/bc_grim2`を最新7日183万判断/182チームで学習、**holdout top1 74.0%**(リポジトリ最高。bc_grim 70.25% / bc_v2 約62%)。
③**探索が21pt悪化させることを発見**し撤去。同一壁で純BC 67.1%/400 vs BCS fixed2 46.0%/200。
④`v5.1g`(Grim合意60枚 + bc_grim2 + 純BC)を提出(sub 55172873, COMPLETE)。
...
## ラダー状況(2026-08-02 01:45 JST、active = 最新2提出のみ)

| 提出 | active | 内容 | score |
|---|---|---|---|
| **v5.1g** (sub 55172873) | **yes** | **bc_grim2 + Grim合意60枚 + 純BC(探索なし)** | **600.0(初期値・読まない)** |
| **v4.3h** (sub 54968749) | **yes** | bc_v2 BCS + Hammer4 Alakazam | **740.5** / 217戦49.8% |
| v4.3a (sub 54731784) | no | bc_v2 BCS + canonical Top Alakazam | **795.7で押し出し** / 315戦49.8% |

⚠️ **v5.1gの初期scoreは読まないこと**。判定は最低80–100戦(約2日、08-04頃)以降。
⚠️ 日付順は v5.1g(08-01) > v4.3h(07-25) > v4.3a(07-15)。**押し出されたのはv4.3a(795.7)**で、
   既知で劣るv4.3h(740.5)が枠に残っている。第2枠の使い道は下の「次のアクション」を参照。
...
2. **第2枠(v4.3h 740.5)の使い道を決める**。候補:
   - **(推奨)`v5.0g`(Grim + bc_grim2 + BCS 8s)を提出** → 同一デッキ・同一モデルで
     **探索の有無だけが違う本番A/B**になる。ローカルの壁は実物より弱いことが既知
     (対Grim ローカル50%に対し本番31%)なので、この判別はラダーの方が信頼できる。
     未決着の仮説A/Bにも本番データで答えが出る。
   - `v4.3a`を再提出して実績795.7を戻す(ただしレートは600から再収束、性能は既知で情報価値は低い)
   - 何もせずv5.1gの結果を待つ
...
4. **探索を救えるか(未決着の本丸)**。安い順に:
   - **ロールアウト方策を確率的にする**(argmax → softmaxサンプリング)。軌跡に多様性が戻り
     相手の別ラインも自然に探索される。Silver & Tesauro "Monte-Carlo Simulation Balancing"の
     「求められるのは強さではなくバランス」に対応する処方箋。
...
5. **`bc_kang2`を作る**(対抗馬への保険)。最新データで`bc_filter_deck --key Kangaskhan`。
...
- (空き) — 08-02 02:00時点で実行中のジョブなし。v5.1g提出済み、次は08-04頃のラダー判定。
